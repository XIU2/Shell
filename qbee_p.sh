...
FOLDER_DOWNLOAD_UNZIP="${FOLDER}/Download/qBittorrent"
FOLDER_UPLOAD="${FOLDER}/Upload"
FILE_OLD_VER="/root/bash/qBittorrentEE/old_ver.txt"
...
_ZIP(){
	cd ${FOLDER_DOWNLOAD}
	7z a -bb0 "qBittorrentEE-v${NEW_VER} 便携版.7z" "${FOLDER_DOWNLOAD_UNZIP}" > /dev/null # 压缩
	rm -rf "${FOLDER_DOWNLOAD_UNZIP}" # 删除前面解压，已经无用文件夹
	[[ ! -e "qBittorrentEE-v${NEW_VER} 便携版.7z" ]] && _NOTICE "ERROR" "qBittorrentEE_v${NEW_VER}压缩失败！"
	[[ ! -e ${FOLDER_UPLOAD} ]] && mkdir "${FOLDER_UPLOAD}" # 如果上传文件夹不存在，就新建
	mv "qBittorrentEE-v${NEW_VER} 便携版.7z" "${FOLDER_UPLOAD}" # 移动到上传文件夹

}

#上传
_UPLOAD(){
	bash ${LZY_PATH} "qBittorrentEE-v${NEW_VER} 便携版.7z" "${FOLDER_UPLOAD}/qBittorrentEE-v${NEW_VER} 便携版.7z" "${FOLDER_ID}"
